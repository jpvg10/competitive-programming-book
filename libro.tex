...
\section{Grafos}
	...
	\subsection{Máximo flujo y mínimo corte}
	...
	Es el algoritmo de Edmonds-Karp, una implementación del método de Ford-Fulkerson que usa BFS para hallar los caminos en el grafo residual. Su complejidad es de \( O(nm^2) \).	
	
	Si hay varios orígenes \( \{s_1, s_2, ...\} \) , se pone un ``super origen'' \( s \), y se agregan aristas \( (s, s_i) \) con capacidad infinita. Análogamente, si hay varios destinos \( \{t_1, t_2, ...\} \), se agrega un ``super destino'' \( t \) y se agregan aristas \( (t_i, t) \) con capacidad infinita.
	
	El máximo flujo es igual al mínimo corte (esto es un teorema). Por ende, este algoritmo halla también el mínimo costo de cortar aristas de manera que \( s \) y \( t \) queden desconectados.
